...
export default function ChatArea(props) {
  ...
  const scrollToBottom = () => {
    if (timelineRef) {
      // DOMの追加描画を確実に待つために setTimeout を使用
      setTimeout(() => {
  ...
  // 🛠️ チャンネルが切り替わった瞬間を検知してスクロール
  createEffect(() => {
    // props.activeChannelName の変化を監視
  ...
  const handleFileChange = (e) => {
    const file = e.target.files[0]; // 修正：[0]で確実にファイル単体を取得
  ...
  const handleSubmit = (e) => {
    ...
    // 🛠️ 送信ボタンが押された瞬間に、強制的に最下部スクロールを実行
    scrollToBottom();
  };
  ...
  return (
    <main style={{ flex: 1, display: "flex", "flex-direction": "column", "background-color": "#ffffff" }}>
      ...
      {/* スクロールエリア（高さ計算のために適切なフレックス・高さを確保） */}
      <div 
        ref={timelineRef}
        style={{ flex: 1, padding: "24px", "overflow-y": "auto", display: "flex", "flex-direction": "column", gap: "20px", height: "0" }}
      >
