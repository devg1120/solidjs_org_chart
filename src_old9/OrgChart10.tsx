...
function recursiveSetId(
  currentNode: OrgNode,
  id: string = "1"
) {
    //console.log("recursiveSetId", id, currentNode)
    let number = 1
    currentNode.id = id;
    if ( currentNode.children ) {
    for ( const c of currentNode.children ) {
          recursiveSetId( c, id + "-" + String(number))
	  number = number + 1
    }
    }
}
  ...
  children: [
    ...
    },
    {
      ...
      children: [
        ...
        { id: "8", box: false, on: "リモート", name: "横山 合", role: "営業担当", level: 3, email: "s.watanabe@example.com", bio: "フットワークの軽さを活かした提案を心がけています。" },
...
recursiveSetId(initialOrgData)
const initExpandedNodes = new Set(["1", "1-1","1-2"]);
...
export const [isSetParent, setIsSetParent] = createSignal(false);
...
export const [expandedNodes, setExpandedNodes] = createSignal<Set<string>>(initExpandedNodes);
...
export const [activeNode, setActiveNode] = createSignal<OrgNode | null>(null);
createEffect(() => {
     if (activeProfile()) {
         setActiveNode(activeProfile)
	 console.log("select")
...
export function OrgChartNode(props: { node: OrgNode }) {
  ...
  const isActive = () =>  {
     if (activeNode()) {
       return props.node.id ==  activeNode().id;
     }
       return false;
     }
  ...
  const isChildOnLabel = () =>  {
     if ( "children" in props.node ) {
       if ( props.node.children.length > 0 ) {
           if ( "on" in props.node.children[0] ) {
             return true;
           }
        }
     }
...
const isCurrentlyOpen = () => {
  ...
  return (
    <div class="org-node-container-top"
        ...
        classList={{  "margin-top" : !isBox() && !isUpperNodeOpen() && isChildOnLabel()}} 
    >
      ...
      */}
      ...
      <div 
        ...
        classList={{  "active-node" : isActive()}} 
        onClick={() => { 
...
*/
  ...
  const handleSelectOrg = (id: string) => {
    ...
    if (single_select) {
        let next = new Set(selectedOrgIds());
        if (next.has(id)) {
          next.delete(id);
        } else {
          next = new Set([id]);
        ...
        return (
          <div class="modal-overlay" onClick={() => { setActiveProfile(null); setModalPos({ x: 0, y: 0 }); }}>
            ...
            >
              ...
              {/* 【A】閲覧モード */}
              <Show when={!isEditing() && !isAddingChild() && !isSetParent()}>
                      ...
                      <div style={{ display: "flex", gap: "8px", "margin-top": "4px" }}>
                        <button 
                          ...
                          onClick={() => {
                            ...
                            (setChartData as any)(...p.parentPath, "children", list);  
                          }}
                        >
                          ▲ 上に移動
                        </button>
                  ...
                  <div class="btn-row">
                    ...
                    <button class="btn-primary" onClick={() => {
                      ...
                      if (isEditing()) {
                        ...
                        setIsEditing(false);setIsAddingChild(false); 
                      } else {
              ...
              {/* 【C】所属変更モード */}
              <Show when={isSetParent()}>
                <div class="modal-header">
                  <h3 style={{ "font-size": "16px", margin: 0, color: "#1e293b" }}> {profile().name}  " 所属の編集" </h3>
                </div>
                <div class="modal-body" style={{ cursor: "default" }}>
  ...
  return (
    <div class="org-chart-wrapper-top">
	  ...
	  */
        ...
        .org-card-body { position: relative }
        /* カードのスタイル（高さ固定） */
      ...
      /*  .org-toggle-btn { background: #ffffff; border: 1px solid #ffffff; border-radius: 4px; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 15px;   cursor: pointer; color: #64748b; padding: 0; } */
        .org-toggle-btn { background: #ffffff; border: 1px solid #ffffff; border-radius: 4px; width: 20px; height: 20px; 
	 position: absolute; ; 
  right: 0;                 /* 右寄せ */
  top: 50%;                 /* 上から50%の位置 */
  transform: translateY(-50%); /* 自身の高さの半分だけ上に戻して上下中央に */
	 font-size: 16px;   cursor: pointer; color: #64748b; padding: 0; }
	...
	*/
        .org-children-container-top > .org-node-container-top:last-child::before { content: ''; position: absolute; left: -30px; top: 40px; bottom: -16px; width: 8px; background-color: #f8fafc; z-index: 2; }
	...
	.active-node { border: solid 2px red; border-radius: 8px;}
	.box { 
